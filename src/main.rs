...
//! Arrow Keys  Move around by off-setting generated noise
...
struct State {
    ...
    noise_scale: f32,
    noise_speed: f32,
    noise_vec: Vec<Box<NoiseFn<noise::Point3<f64>>>>,
    z_offset: f32,
    show_help: bool,
}

impl State {
    fn active_noise(&self) -> &NoiseFn<noise::Point3<f64>> {
...
impl EventHandler for State {
    fn update(&mut self, ctx: &mut Context) -> GameResult<()> {
        ...
        for y in 0..GRID_SIZE_Y {
            ...
            for x in 0..GRID_SIZE_X {
                let angle: f32 = self.active_noise().get([f64::from(x_offset), f64::from(y_offset), f64::from(self.z_offset)]) as f32 * TWO_PI;
    ...
    fn key_down_event(&mut self, ctx: &mut Context, keycode: Keycode, _keymod: Mod, repeat: bool) {
        match keycode {
            ...
            Keycode::LeftBracket => self.noise_speed -= DEFAULT_NOISE_SPEED_INCREMENT,
            Keycode::RightBracket => self.noise_speed += DEFAULT_NOISE_SPEED_INCREMENT,
...
fn gen_line_segments(noise: &NoiseFn<noise::Point3<f64>>, z_offset: f32) -> Vec<LineSegment> {
...
fn gen_noise_vec() -> Vec<Box<NoiseFn<noise::Point3<f64>>>> {
    vec!(
